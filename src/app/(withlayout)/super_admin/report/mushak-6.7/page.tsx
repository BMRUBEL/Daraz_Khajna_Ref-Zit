...
} from "@/redux/api/reportApi/mushakSixSevenApi";
...
const MushakSixSeven = () => {
  ...
  const [creditParameter, setCreditParameter] = useState(null);
  ...
  // drop down data
  const { data: compdata } = useSixSevenCompDropdownQuery({ ...{}, });
  const { data: branchdata } = useSixSevenBranchDropdownQuery({ ...{}, });
  const { data: storedata } = useSixSevenStoreDropdownQuery({  ...{}, });
  const { data: vatdata } = useSixSevenVatDropdownQuery({  ...{}, });
  const { data: creditdata } = useSixSevenCreditDropdownQuery({  ...{}, });
  ...
  const creditNoteOptions =
    creditdata &&
    creditdata?.result.filter((credit:any)=>  credit?.credit_note_id!==null).map((credit: any) => {
      return {
        label: credit?.credit_note_no,
        value: parseInt(credit?.credit_note_id),
      };
    });
  ...
  const handleFetchData = () => {
    if (selectedCompany == null || selectedBranch == null || selectedStore == null || selectedVatMonth == null || selectedCreditId == null ) {
      ...
      setCreditParameter(selectedCreditId)
      setShowReport(true)
...
// showing Pdf
  const fetchPdfData = async (
    ...
    selectedCreditId :any,
    ) => {
   console.log(selectedCompany, "pdf parameter");
   try {
     const response = await fetch(
       `http://192.168.10.10:8181/report/api/v1/mushak-six-seven/gen-pdf?companyId=${selectedCompany}&branchId=${selectedBranch}&storeId=${selectedStore}&vatMonthId=${selectedVatMonth}&creditNoteId=${selectedCreditId}`,
       {
         headers: {
 ...
 const handleFetchPdf = () => {
    fetchPdfData(selectedCompany,selectedBranch,selectedStore,selectedVatMonth,selectedCreditId);
  };
                  ...
                  >
                    ...
                    >
                      <DRZSelectField
                        ...
                        label="Credit Note:  "
                        placeholder="Select Credit Note ID"
                        onSearch={onSearch}
                        filterOption={filterOption}
                        onChange={(value: any) =>
          ...
          {showReport ? (
            <tbody>
                ...
                <td colSpan={12}>
                  <div className="grid grid-cols-12 grid-flow-col gap-0  border p-3 mb-5">
                    ...
                    <div className="col-start-11 col-end-13 text-center">
                      <p className="font-bold">মূসক-৬.৭</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-5 grid-flow-col gap-0 border p-2">
                    ...
                    <div className="col-start-4 col-end-6 flex justify-end">
                      <div className="text-right flex flex-col">
                        <div className="overflow-hidden"
                           style={{marginBottom:"15px"}}>
                          <strong>ক্রেডিট নোট নম্বর:</strong> {rightOne}
                        </div>
                        <div className="overflow-hidden">
                          <strong>চালানপত্র নম্বর:</strong> {rightOne}
                        </div>
                        <div className="overflow-hidden">
              ...
              >
                ...
                >
                  পণ্য বা সেবার (প্রযোজ্য ক্ষেত্রে ব্র্যান্ড নাম সহ বর্ণনা)
                </th>
