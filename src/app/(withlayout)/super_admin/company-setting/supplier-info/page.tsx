...
import { useGetAllSupplierInfoQuery } from "@/redux/api/companyApi/supplierInfoApi";

import { useDebounced } from "@/redux/hook";

import { EditOutlined, EyeOutlined, SwitcherOutlined } from "@ant-design/icons";
...
const CustomerSetup = () => {
  ...
  // get department data
  const { data: supplierData, isLoading } = useGetAllSupplierInfoQuery({
    ...query,
    sortField: sortBy,
  ...
  const columns = [
    ...
    {
      title: "Supplier Name BN",
      dataIndex: "customerNameBn",
      align: "center",
    },
    {
      title: "Supplier Bin Number",
      dataIndex: "customerBinNumber",
      align: "center",
    },
    {
      title: "Supplier Bin Number Bn",
      dataIndex: "customerBinNumberBn",
      align: "center",
    },
    ...
    },
    {
      ...
      render: (record: any) => (
        ...
        >
          <Link
            href={`/super_admin/company-setting/supplier-info/view/${record}`}
          >
            <Button
              style={{
                fontSize: "13px",
          ...
          <Link
            href={`/super_admin/company-setting/supplier-info/edit/${record}`}
          >
            <Button
              className="bg-[#FF5100]"
              style={{
    ...
    <>
      ...
      >
        ...
        <div>
          <ActionBar>
            <Link
              href="/super_admin/company-setting/supplier-info/create"
          ...
          {isLoading ? (
            ...
            <UMTable
              loading={isLoading}
              columns={columns}
              dataSource={supplierData?.result?.content}
              onTableChange={onTableChange}
              pageSize={size}
              showSizeChanger={true}
              onPaginationChange={onPaginationChange}
              showPagination={true}
              totalPages={supplierData?.result.totalElements}
            />
          )}
        </div>
      </div>
