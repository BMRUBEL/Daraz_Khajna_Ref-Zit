...
import { useItemInfoQuery } from "@/redux/api/housekeepingApi/itemInfoApi";
...
const ManageAdmin = () => {
  ...
  // get department data
  const { data: transactionData, isLoading } = useItemInfoQuery({
    ...query,
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: any) => (
        ...
        >
          <Link href={`/super_admin/company-setting/items/item-info/view/${record}`}>
            <Button
              style={{
                fontSize: "13px",
          ...
          <Link href={`/super_admin/company-setting/items/item-info/edit/${record}`}>
            <Button
              className="bg-[#FF5100]"
              style={{
                fontSize: "13px",
  ...
  return (
    <>
      ...
      >
        <UMBreadCrumb pageName="Item Info" />

        <div>
          <ActionBar>
            <Link
              href="/super_admin/company-setting/items/item-info/create"
