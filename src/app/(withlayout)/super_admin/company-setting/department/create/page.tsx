...
import { useAddDepartmentMutation, useSectionDropDownQuery } from "@/redux/api/companyApi/departmentApi";
...
const CreateDepartmentPage = () => {
  ...
  const [addDepartment, { data: createData }] = useAddDepartmentMutation();
  const { data: datasource } = useSectionDropDownQuery({
  });
  console.log(createData, "Create Department");
    ...
    try {
      ...
      if (res) {
        if ('data' in res && res.data && res.data.result) {
          message.success("Company Department created successfully!");
          router.push(
            `/super_admin/company-setting/department/view/${res?.data?.result?.id}`
          );
          setLoading(false);
        } else {
  ...
  return (
    ...
    >
      <Link href={`/super_admin/company-setting/department`}>
        <UMBreadCrumb pageName="Department" lastName="Create" />
      </Link>
      <div
        style={{
          padding: "20px",
