...
import { useSectionQuery } from "@/redux/api/companyApi/sectionApi";
...
const ManageAdmin = () => {
  ...
  // get department data
  const { data: sectionData, isLoading } = useSectionQuery({
    ...query,
    page: page,
    size: size,
    sortField: sortBy,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: any) => (
        ...
        >
          <Link
            href={`/super_admin/company-setting/section/view/${record}`}
          >
            <Button
              style={{
                fontSize: "13px",
          ...
          <Link
            href={`/super_admin/company-setting/section/edit/${record}`}
          >
            <Button
              className="bg-[#FF5100]"
              style={{
  ...
  return (
    <>
      ...
      >
        <UMBreadCrumb pageName="Section" />

        <div>
          <ActionBar>
            <Link
              href="/super_admin/company-setting/section/create"
          ...
          {isLoading ? (
            ...
            <UMTable
              loading={isLoading}
              columns={columns}
              dataSource={sectionData?.result?.content}
              onTableChange={onTableChange}
              totalPages={sectionData?.result?.totalElements}
              pageSize={sectionData?.result?.size}
