...
const CreateDesignationPage = () => {
  ...
  const [addTransaction, { data: createData }] = useAddDesignationMutation();
    ...
    try {
      ...
      if (res) {
        if ('data' in res && res.data && res.data.result) {
          ...
          router.push(
            `/super_admin/company-setting/designation/view/${res.data.result.id}`
          );
          setLoading(false);
        } else {
  ...
  return (
    ...
    >
         ...
         <UMBreadCrumb pageName="Designation" lastName="Create" />
         </Link>
      <div
        style={{
          padding: "20px",
      ...
      >
        <Form submitHandler={onSubmit} resolver={yupResolver(designationSchema)}>
          <div
            style={{
              padding: "0px",
              marginTop: "12px",
