...
import { useDesignationQuery } from "@/redux/api/companyApi/designationApi";
...
const ManageAdmin = () => {
  ...
  // get department data
  const { data: designationData, isLoading } = useDesignationQuery({
    ...query,
    page: page,
    size: size,
    sortField: sortBy,
  ...
  const columns = [
    ...
    {
      title: "Designation Name Bn",
      dataIndex: "desigNameBn",
      width: 300,
      align: "center",
    },
    ...
    },
    {
      ...
      render: (record: any) => (
        ...
        >
          <Link
            href={`/super_admin/company-setting/designation/view/${record}`}
          >
            <Button
              style={{
                fontSize: "13px",
          ...
          <Link
            href={`/super_admin/company-setting/designation/edit/${record}`}
          >
            <Button
              className="bg-[#FF5100]"
              style={{
  ...
  return (
    <>
      ...
      >
        <UMBreadCrumb pageName="Designation" />

        <div>
          <ActionBar>
            <Link
              href="/super_admin/company-setting/designation/create"
          ...
          {isLoading ? (
            ...
            <UMTable
              loading={isLoading}
              columns={columns}
              dataSource={designationData?.result?.content}
              totalPages={designationData?.result?.totalElements}
              onTableChange={onTableChange}
              pageSize={designationData?.result?.size}
