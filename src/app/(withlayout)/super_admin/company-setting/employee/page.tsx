...
import { useCompanyEmployeeQuery } from "@/redux/api/companyApi/companyEmployeeApi";
...
const CompanyInfo = () => {
  ...
  // get department data
  const { data: companyEmployeeData, isLoading } = useCompanyEmployeeQuery({
    ...query,
    page: page,
    size: size,
    sortField: sortBy,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: any) => (
        ...
        >
          <Link
            href={`/super_admin/company-setting/employee/view/${record}`}
          >
            <Button
              style={{
                fontSize: "13px",
          ...
          <Link
            href={`/super_admin/company-setting/employee/edit/${record}`}
          >
            <Button
              className="bg-[#FF5100]"
              style={{
  ...
  return (
    <>
      ...
      >
        <UMBreadCrumb pageName="Company Employee" />

        <div>
          <ActionBar>
            <Link
              href="/super_admin/company-setting/employee/create"
          ...
          {isLoading ? (
            ...
            <UMTable
              loading={isLoading}
              columns={columns}
              dataSource={companyEmployeeData?.result?.content}
              onTableChange={onTableChange}
              totalPages={companyEmployeeData?.result?.totalElements}
              pageSize={companyEmployeeData?.result?.size}
