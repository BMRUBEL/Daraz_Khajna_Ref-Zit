...
import { othersadjustmentSchema } from "@/schemas/othersadjustmentSchema";
...
const EditOtherAdjustment = ({ params }: any) => {
  // state
  ...
  const [updateOtherAdjustment] = useUpdateOtheradjustmentMutation();
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      setLoading(true);
      const res = await updateOtherAdjustment({
        id,
        body: values,
      }).unwrap();
      ...
      if (res) {
        if(res?.code===400){
          ...
          message.success("Others Adjustments updated successfully!");
          router.push(`/super_admin/hk/others-adjustments/view/${id}`);
        }
  ...
  return (
    ...
    >
        ...
        <UMBreadCrumb pageName="Others Adjustment" lastName="Update" link={`/super_admin/hk/others-adjustments`} />
      ...
      >
        ...
        resolver={yupResolver(othersadjustmentSchema)}
...
export default EditOtherAdjustment;
