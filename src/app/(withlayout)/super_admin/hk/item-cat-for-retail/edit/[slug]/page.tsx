...
} from "@/redux/api/housekeepingApi/itemCatForRetailApi";
...
const EditItemCatRetail = ({ params }: any) => {
  // state
  ...
  const { data = [], isLoading } = useGetSingleitemcategoryQuery(id, {});
  ...
  useEffect(() => {
    setDefaultValue({
      ...
      itemCatRetailName: data.result?.itemCatRetailName,
      itemCatRetailNameBn: data.result?.itemCatRetailNameBn,
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      console.log(errorCat, "After getting res")
      ...
      if (res) {
        if(res?.result.code===400){
          ...
          message.success("Item Retail Updated successfully!");
          router.push(`/super_admin/hk/item-cat-for-retail/view/${id}`);
        }
  ...
  return (
    ...
    >
      ...
      <UMBreadCrumb pageName="Item Cat For Retail" lastName="Update" link={`/super_admin/hk/item-cat-for-retail`}  />
      ...
      >
        <Form submitHandler={onSubmit} resolver={yupResolver(itemCategorySchema)} defaultValues={defaultValue}>
          <div
            style={{
              padding: "0px",
              marginTop: "12px",
...
export default EditItemCatRetail;
