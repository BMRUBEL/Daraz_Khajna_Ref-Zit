...
const EditChallanType = ({ params }: any) => {
  // state
  ...
  const [updateChallanType] = useUpdateChallanTypeMutation();
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      setLoading(true);
      const res = await updateChallanType({
        id,
        body: values,
      }).unwrap();
      ...
      if (res) {
        if(res?.code===400){
          ...
          message.success("Challan Type updated successfully!");
          router.push(`/super_admin/hk/challan-type/view/${id}`);
        }
  ...
  return (
    ...
    >
        ...
        <UMBreadCrumb pageName="Challan Type" lastName="Update" link={`/super_admin/hk/challan-type`} />
      ...
      >
        ...
        resolver={yupResolver(challanTypeSchema)}
