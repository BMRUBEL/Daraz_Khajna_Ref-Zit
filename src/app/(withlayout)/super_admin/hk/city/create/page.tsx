...
const CreateCityPage = () => {
  ...
  const [addCity, { data: createData }] = useAddCityMutation();
  console.log(createData, "createTransaction");

  const router = useRouter();
  const { data: datasource } = useCitydropdownQuery({
    ...{},
  });
  console.log(datasource, "source");
  const cityOptions =
    datasource &&
    datasource?.result.map((city: any) => {
      return {
        label: city?.name,
  ...
  console.log(cityOptions, "options");
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (res) {
        ...
        router.push(`/super_admin/hk/city/view/${res.data.result.id}`);
        setLoading(false);
        }else{
  ...
  return (
    ...
    >
        ...
        <UMBreadCrumb pageName="City" lastName="Create" link="/super_admin/hk/city" />
      ...
      >
        <Form submitHandler={onSubmit} resolver={yupResolver(citySchema)}>
          <div
            style={{
              padding: "0px",
              marginTop: "12px",
