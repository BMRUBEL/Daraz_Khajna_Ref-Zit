...
const EditDistrict = ({ params }: any) => {
  ...
  // console.log(sourceId);
  const [updateTransaction] = useUpdateDistrictMutation();
  ...
  useEffect(() => {
    setDefaultValue({
      ...
      divisionId: parseInt(data.result?.divisionId),
      // lat: data.result?.lat,
    ...
    try {
      ...
      if (res) {
        if(res?.code===400){
          ...
          router.push(`/super_admin/hk/district/view/${id}`);
        }
  ...
  return (
    ...
    >
      ...
      <UMBreadCrumb pageName="District" lastName="Update" link={`/super_admin/hk/district`} />
