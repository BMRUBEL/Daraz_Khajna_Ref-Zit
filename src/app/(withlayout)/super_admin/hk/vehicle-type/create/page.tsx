...
import { useAddVehcleTypeMutation } from "@/redux/api/housekeepingApi/vehicleTypeApi";
...
const CreateVehicleTypePage = () => {
  const [loading, setLoading] = useState(false);

  const { data, isLoading } = useDepartmentsQuery({ limit: 100, page: 1 });

  const [addVehicle, { data: createData }] = useAddVehcleTypeMutation();
    ...
    try {
      ...
      if (res) {
        if(res?.data?.code===400){
          ...
          router.push(`/super_admin/hk/vehicle-type/view/${res.data.result.id}`);
          setLoading(false);
        }else{
  ...
  return (
    ...
    >
        ...
        <UMBreadCrumb pageName="Vehicle Type" lastName="Create" link={`/super_admin/hk/vehicle-type`} />
            ...
            >
              ...
              >
                <FormInput
                  ...
                  placeholder="Vehicle Type Name BN"
                  required
                />
              </Col>
              <Col
