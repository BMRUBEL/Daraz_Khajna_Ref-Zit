...
const CreateBankInfoPage = () => {
  const [loading, setLoading] = useState(false);
  const { data, isLoading } = useDepartmentsQuery({ limit: 100, page: 1 });

  const [addBankInfo, { data: createData }] = useAddBankinfoMutation();
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      setLoading(true);
      const res = await addBankInfo(values);
      ...
      if (res) {
        if(res?.data?.code===400){
          ...
          router.push(`/super_admin/hk/bank-info/view/${res.data.result.id}`);
          setLoading(false);
        } else {
  ...
  return (
    ...
    >
        ...
        <UMBreadCrumb pageName="Bank Info " lastName="Create" link="/super_admin/hk/bank-info" />
      ...
      >
        <Form submitHandler={onSubmit} resolver={yupResolver(bankInfoSchema)}>
          <div
            style={{
              padding: "0px",
              marginTop: "12px",
...
export default CreateBankInfoPage;
