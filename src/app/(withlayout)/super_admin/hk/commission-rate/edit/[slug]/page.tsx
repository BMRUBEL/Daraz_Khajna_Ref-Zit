...
const EditCommissionRate = ({ params }: any) => {
  // state
  ...
  const { data = [], isLoading } = useGetSingleCommissionrateQuery(id, {});
  // const sourceId: any = data[0]?.tran_source_type_id;
  // console.log(sourceId);

  // console.log(sourceId);
  const [updateCommissionrate] = useUpdateCommissionRateMutation();
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      setLoading(true);
      const res = await updateCommissionrate({
        id,
        body: values,
      }).unwrap();
      ...
      if (res) {
        if(res?.code===400){
          ...
          router.push(`/super_admin/hk/commission-rate/view/${id}`);
        }
  ...
  return (
    ...
    >
        ...
        <UMBreadCrumb pageName="Commission Rate" lastName="Update" link={`/super_admin/hk/commission-rate`} />
...
export default EditCommissionRate;
