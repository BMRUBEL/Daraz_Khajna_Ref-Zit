...
const EditLanguage = ({ params }: any) => {
  ...
  const { data = [], isLoading } = useGetSingleLanguageQuery(id, {});
  ...
  useEffect(() => {
    setDefaultValue({
      ...
      appLangCode: data.result?.appLangCode,
      rtl: data.result?.rtl,
    ...
    try {
      ...
      if (res) {
        if(res?.code===400){
          ...
          router.push(`/super_admin/hk/language/view/${id}`);
        }
  ...
  return (
    ...
    >
        ...
        <UMBreadCrumb pageName="Languages" lastName="Update" link={`/super_admin/hk/language`} />
